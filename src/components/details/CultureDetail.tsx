...
const CultureDetail: React.FC<DetailProps<CultureItem>> = ({ item, lang, t }) => {
  return (
    <>
      ...
      <div className="col-span-12 mt-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="p-8 border border-white/5 bg-white/2 rounded-3xl flex flex-col justify-center items-center">
            ...
            <div className="flex flex-wrap justify-center gap-12">
              {t(item.details.sports).map((s: string) => <span key={s} className="text-white font-black uppercase text-2xl lg:text-4xl tracking-tighter italic opacity-80">{s}</span>)}
            </div>
        </div>
